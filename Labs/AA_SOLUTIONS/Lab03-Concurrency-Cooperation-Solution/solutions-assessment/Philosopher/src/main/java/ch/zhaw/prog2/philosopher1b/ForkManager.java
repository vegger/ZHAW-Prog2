package ch.zhaw.prog2.philosopher1b;
...
import static ch.zhaw.prog2.philosopher1b.ForkManager.ForkState.*;
...
class ForkManager {
    ...
    public void acquireForks(int philosopherId) throws InterruptedException {
        ...
        try {
            ...
            while (leftFork.state != FREE || rightFork.state != FREE) {
                // wait for the one not free
                if (leftFork.state != FREE) {
                    leftFork.forkFree.await();
                } else {
                    rightFork.forkFree.await();
                }
            }
    ...
    public void releaseForks(int philosopherId) throws InterruptedException {
        try {
            managerLock.lock();
            // release both forks and signal waiting philosophers
