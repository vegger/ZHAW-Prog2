...
public class MandelbrotCompletionServiceProcessor extends MandelbrotProcessor {
     ...
     */
    public MandelbrotCompletionServiceProcessor(MandelbrotProcessorListener processorListener, int width, int height) {
    ...
    public void startProcessing(int numThreads) {
        ...
        tasksRemaining = 0;  // Records how many of the threads are still running
        super.startTime = System.currentTimeMillis();

        // Start the the executor service with the given number of threads.
        executor = Executors.newFixedThreadPool(numThreads);
        // store returned Futures in a list
        CompletionService<ImageRow> completionService = new ExecutorCompletionService<>(executor);
        // Create a task for the row and submit it to the executor service
        for (int row = 0; row < height; row++) {
            completionService.submit(new MandelbrotTask(row));
            tasksRemaining++;
        }
        long duration = System.currentTimeMillis()-startTime;
        System.out.println("Tasks submitted after " + duration + "ms");

        // get completed results from CompletionService and send them to the Listener (GUI)
        while (tasksRemaining > 0) {
            try {
                Future<ImageRow> imageRowFuture = completionService.take();
                ImageRow imageRow = imageRowFuture.get();
