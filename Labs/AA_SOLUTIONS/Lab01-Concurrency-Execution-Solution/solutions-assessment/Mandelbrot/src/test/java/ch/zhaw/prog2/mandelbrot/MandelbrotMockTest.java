...
public class MandelbrotMockTest {
    ...
    public void testMandelbrotTaskProcessorListenerCalls() throws InterruptedException {
        ...
        String reason = String.format("rowProcessed should be called %d times", height);
        verify(listener, times(height).description(reason)).rowProcessed(any(ImageRow.class));

        reason = "MandelbrotTaskProcessor.stopProcessing has to be called exactly once";
        verify(listener, times(1).description(reason)).processingStopped(anyLong());
    }
